...
|---|-------------|--------|--------|-------------|
| 1.1 | **`tsc --noEmit` post-apply verification** | S | ⭐⭐⭐ | Run TypeScript compiler on all generated files before committing the WorkspaceEdit. Abort and show errors if any file fails to compile. |
| 1.2 | **Exact AST-walk Cognitive Complexity** | S | ⭐⭐ | Replace line-level CogCC approximation with a full TypeScript AST traversal for exact nesting depth at every node. |
| 1.3 | **Function-level region splitting** | M | ⭐⭐⭐ | Split regions that contain multiple exported functions into one region per function, not one region per top-level statement. |
| 1.4 | **Re-export chain resolution** | S | ⭐⭐ | Follow `export { A } from './b'` chains in the workspace graph so the `symbolExporters` map includes transitively re-exported symbols. |
| 1.5 | **Semantic type resolution via TS type-checker** | L | ⭐⭐⭐ | Use `ts.createProgram` + `TypeChecker` to resolve generic constraints, conditional types, `satisfies`, and `infer` — not just structural heuristics. |
| 1.6 | **Dead code detection** | M | ⭐⭐ | Mark exports that are never imported anywhere in the workspace (cross-file dead export). Currently only detects dead exports within the same file. |
| 1.7 | **Duplicate function detection across workspace** | M | ⭐⭐⭐ | Compare function signatures and body fingerprints across all workspace files. Suggest deduplication into a shared utility. |
| 1.8 | **Copy-paste detector (type-2 clones)** | M | ⭐⭐ | Detect near-duplicate code blocks that differ only in variable names (type-2 clones) using normalised AST comparison. |
| 1.9 | **Halstead-calibrated per-function thresholds** | S | ⭐⭐ | Apply the Halstead calibration at the function level, not just the file level, so heavily-loaded functions within a simple file are still flagged. |
| 1.10 | **LCOM4 cohesion metric** | M | ⭐⭐ | Implement the full Lack of Cohesion in Methods 4 metric using the actual call graph between methods within a class, replacing the current approximation. |
...
| 3.2 | **AI extraction reasoning** | S | ⭐⭐ | For each extraction candidate, call Claude to produce a one-paragraph plain-English explanation of *why* this region should be extracted — beyond the mechanical oracle reasons. |
