...
function extractVueBlocks(src: string): VueBlocks {
    const scriptSetupM  = src.match(/<script\s[^>]*setup[^>]*>([\s\S]*?)<\/script>/i);
    const scriptLegacyM = src.match(/<script(?!\s[^>]*setup)[^>]*>([\s\S]*?)<\/script>/i);
...
export function detectVueSmells(src: string): FrameworkSmell[] {
    ...
    if (/defineProps\s*\(\s*\[/.test(script)) {
        smells.push({
            ...
            recommendation: 'Use defineProps<{ prop: Type }>() with TypeScript generic for full type safety',
            autoFixable: false, framework: 'vue', line: 1,
        });
    }

    // watch without cleanup
    if (/\bwatch\s*\(/.test(script) && !/onWatcherCleanup|onCleanup/.test(script)) {
        if (/setTimeout|setInterval|fetch\(|addEventListener/.test(script)) {
            smells.push({
                name: 'watch Without Cleanup',
                severity: 'high',
                description: 'watch callback uses async operations or timers but has no cleanup',
                recommendation: 'Return a cleanup function from the watcher, or use watchEffect with onCleanup()',
                autoFixable: false, framework: 'vue', line: 1,
            });
        }
    }
    ...
    if (/addEventListener/.test(script) && !/onUnmounted/.test(script)) {
        smells.push({
            ...
            severity: 'high',
            description: 'Event listener added but no onUnmounted cleanup found — memory leak',
            recommendation: 'Call removeEventListener inside onUnmounted(() => { ... })',
            autoFixable: false, framework: 'vue', line: 1,
        });
    }
    ...
    if (scriptLegacy) {
        ...
        if (dataMatch && dataMatch[1].split('\n').length > 20) {
            smells.push({
                ...
                severity: 'medium',
                description: 'data() returns many properties — hard to track reactivity',
                recommendation: 'Migrate to Composition API with individual ref() / reactive() declarations',
                autoFixable: false, framework: 'vue', line: 1,
            });
        }
        ...
        if (apiCount >= 5) {
            smells.push({
                ...
                recommendation: 'Migrate to Composition API and extract composables for each concern',
                autoFixable: false, framework: 'vue', line: 1,
            });
        }
    }
    ...
    if (/v-for[^>]+v-if|v-if[^>]+v-for/.test(template)) {
        smells.push({
            ...
            severity: 'high',
            description: 'v-if and v-for on the same element causes unnecessary re-evaluation',
            recommendation: 'Wrap with <template v-for> and put v-if on the inner element, or filter the array in computed',
            autoFixable: false, framework: 'vue', line: 1,
        });
    }

    // Direct mutation of props in template
    if (/\$emit\s*\(/.test(template) === false && /\bprops\.\w+\s*=/.test(template)) {
        smells.push({
            name: 'Direct Prop Mutation in Template',
            severity: 'critical',
            description: 'Props are being mutated directly in the template',
            recommendation: 'Emit an event to the parent to update the prop: emit("update:propName", newValue)',
            autoFixable: false, framework: 'vue', line: 1,
        });
    }

    // Missing key on v-for
    if (/v-for/.test(template) && !/v-for[^>]+:key|:key[^>]+v-for/.test(template)) {
        smells.push({
            name: 'Missing :key on v-for',
            severity: 'medium',
            description: 'v-for without :key causes inefficient DOM diffing',
            recommendation: 'Always add :key with a unique identifier: <div v-for="item in list" :key="item.id">',
            autoFixable: false, framework: 'vue', line: 1,
        });
    }
    ...
    if (count(template, /\{\{[^}]{60,}\}\}/g) > 2) {
        smells.push({
            ...
            recommendation: 'Move complex expressions to computed properties for readability and caching',
            autoFixable: false, framework: 'vue', line: 1,
        });
    }
    ...
    if (src.split('\n').length > 300) {
        smells.push({
            ...
            recommendation: 'Extract sub-components and composables. Keep SFCs under 200 lines.',
            autoFixable: true, framework: 'vue', line: 1,
        });
    }
...
export function detectAngularSmells(src: string): FrameworkSmell[] {
    ...
    if (hasComponent) {
        ...
        if (inlineTemplate && inlineTemplate[1].split('\n').length > 5) {
            smells.push({
                ...
                recommendation: 'Move template to a separate .html file using templateUrl',
                autoFixable: false, framework: 'angular', line: 1,
            });
        }
        ...
        if (/styles\s*:\s*\[`/.test(src)) {
            smells.push({
                ...
                recommendation: 'Move styles to a separate .scss/.css file using styleUrls',
                autoFixable: false, framework: 'angular', line: 1,
            });
        }
        ...
        if (!(/changeDetection\s*:\s*ChangeDetectionStrategy\.OnPush/.test(src))) {
            smells.push({
                ...
                recommendation: 'Add changeDetection: ChangeDetectionStrategy.OnPush for better performance',
                autoFixable: false, framework: 'angular', line: 1,
            });
        }
    }
    ...
    if (/@Injectable\s*\(/.test(src)) {
        ...
        if (!(/providedIn\s*:/.test(src))) {
            smells.push({
                ...
                severity: 'high',
                description: '@Injectable without providedIn creates a non-tree-shakable service',
    ...
    if (hasSubscribe && !hasOnDestroy && !hasTakeUntil) {
        smells.push({
            ...
            severity: 'critical',
            description: 'Component subscribes to observables but does not implement OnDestroy or use takeUntil',
            recommendation: 'Implement OnDestroy and unsubscribe, or use takeUntilDestroyed() / AsyncPipe',
            autoFixable: false, framework: 'angular', line: 1,
        });
    }
    ...
    const ngOnInitMatch = src.match(/ngOnInit\s*\(\s*\)\s*\{([\s\S]*?)(?=\n\s{2}\w|\n\})/);
    if (ngOnInitMatch) {
        ...
        if (initLines > 15) {
            smells.push({
                ...
                recommendation: 'Extract service calls and setup logic into private methods called from ngOnInit',
                autoFixable: false, framework: 'angular', line: 1,
            });
        }
    }
    ...
    if (/document\.getElementById|document\.querySelector(?!All)/.test(src)) {
        smells.push({
            ...
            severity: 'high',
            description: 'Using document.getElementById/querySelector bypasses Angular rendering',
            recommendation: 'Use @ViewChild with ElementRef, or Renderer2 for DOM manipulation',
            autoFixable: false, framework: 'angular', line: 1,
        });
    }
    ...
    if (count(src, /:\s*any\b/g) > 3) {
        smells.push({
            ...
            recommendation: 'Replace any with proper types or generics. Angular works best with strict typing.',
            autoFixable: false, framework: 'angular', line: 1,
        });
    }
    ...
    if (lines.length > 250) {
        smells.push({
            ...
            recommendation: 'Extract business logic into dedicated services; keep components lean',
            autoFixable: true, framework: 'angular', line: 1,
        });
    }
...
export function detectSvelteSmells(src: string): FrameworkSmell[] {
    ...
    if (reactiveCount > 6) {
        smells.push({
            ...
            recommendation: 'Extract groups of related reactive statements into a separate .svelte component or store',
            autoFixable: false, framework: 'svelte', line: 1,
        });
    }
    ...
    const hasOnDestroy  = /import\s*\{[^}]*onDestroy/.test(script) || /onDestroy\s*\(/.test(script);
    ...
    if (hasSubscribe && !hasOnDestroy && !hasAutoSub) {
        smells.push({
            ...
            severity: 'critical',
            description: 'Manually subscribing to a store without onDestroy cleanup causes memory leaks',
            recommendation: 'Use the $ prefix for auto-subscription ($store), or call unsubscribe inside onDestroy',
            autoFixable: false, framework: 'svelte', line: 1,
        });
    }
    ...
    if (/\$\w+\s*=\s*/.test(template) && /writable/.test(script)) {
        smells.push({
            ...
            severity: 'low',
            description: 'Assigning to $store directly in the template — bypasses update notification patterns',
            recommendation: 'Use store.set() or store.update() to mutate stores explicitly',
            autoFixable: false, framework: 'svelte', line: 1,
        });
    }
    ...
    if (/{#each/.test(template) && !/{#each[^}]+(key)/.test(template)) {
        smells.push({
            ...
            severity: 'medium',
            description: '{#each} block without a key expression causes inefficient DOM updates',
            recommendation: 'Add a key: {#each items as item (item.id)} for keyed diffing',
            autoFixable: false, framework: 'svelte', line: 1,
        });
    }
    ...
    if (/getContext/.test(script) && !/setContext/.test(src)) {
        smells.push({
            ...
            recommendation: 'Ensure a parent component calls setContext() with the same key before this component mounts',
            autoFixable: false, framework: 'svelte', line: 1,
        });
    }
    ...
    if (count(template, /on:\w+\s*=\s*\{[^}]{40,}\}/g) > 3) {
        smells.push({
            ...
            recommendation: 'Move handler logic to named functions in the <script> block',
            autoFixable: false, framework: 'svelte', line: 1,
        });
    }
    ...
    if (totalLines > 200) {
        smells.push({
            ...
            recommendation: 'Extract child components and stores. Svelte components work best under 150 lines.',
            autoFixable: true, framework: 'svelte', line: 1,
        });
    }
...
export type FrameworkKind = 'vue' | 'angular' | 'svelte' | 'react' | 'none';

export function detectFrameworkKind(src: string, filePath: string): FrameworkKind {
    if (filePath.endsWith('.vue'))    return 'vue';
    if (filePath.endsWith('.svelte')) return 'svelte';
    if (/@Component\s*\(|@NgModule\s*\(|@Injectable\s*\(/.test(src)) return 'angular';
    if (/from ['"]react['"]/.test(src) || /useState|useEffect/.test(src)) return 'react';
    return 'none';
}
...
export function detectFrameworkSmells(
    ...
    switch (kind) {
        case 'vue':     return detectVueSmells(src);
        case 'angular': return detectAngularSmells(src);
        case 'svelte':  return detectSvelteSmells(src);
        default:        return [];
    }
}
