...
function classifySymbolFlags(flags: ts.SymbolFlags): { isType: boolean; isValue: boolean } {
        ...
        if (tsConfigPath && fs.existsSync(tsConfigPath)) {
            ...
            if (!error && config) {
                const parsed = ts.parseJsonConfigFileContent(
                    config, ts.sys, path.dirname(tsConfigPath)
                );
...
export class SemanticTypeResolver {
    ...
    resolveFile(
        ...
        ts.forEachChild(sf, (node) => {
            ...
            for (const name of names) {
                const sym = checker.getSymbolAtLocation(this._getNameNode(node, name, sf) ?? node);
                if (!sym) continue;
                ...
                const info: SemanticSymbolInfo = { name, isTypeOnly: isType && !isValue, flags, typeText };
    ...
    private _getNameNode(
        ...
        if (ts.isFunctionDeclaration(node) && node.name?.text === name) return node.name;
        if (ts.isClassDeclaration(node) && node.name?.text === name) return node.name;
        if (ts.isInterfaceDeclaration(node) && node.name.text === name) return node.name;
        if (ts.isTypeAliasDeclaration(node) && node.name.text === name) return node.name;
        if (ts.isEnumDeclaration(node) && node.name.text === name) return node.name;
        if (ts.isModuleDeclaration(node) && ts.isIdentifier(node.name) && node.name.text === name) return node.name;
        if (ts.isVariableStatement(node)) {
    ...
    private _getTypeText(checker: ts.TypeChecker, sym: ts.Symbol, sf: ts.SourceFile): string {
        try {
