...
const SUPPORTED_EXTS = new Set(['.ts', '.tsx', '.js', '.jsx', '.mts', '.cts', '.mjs']);
...
const IGNORED_DIRS = new Set([
    'node_modules', 'dist', 'out', 'build', '.next', '.nuxt',
    'coverage', '.git', '.vscode', '__pycache__', '.turbo',
]);
...
function detectFramework(src: string, ext: string): WorkspaceFileRecord['framework'] {
...
function scanFile(filePath: string, workspaceRoot: string): WorkspaceFileRecord | null {
    try {
        ...
        const exportRe = /^export\s+(?:default\s+)?(?:const|function|class|interface|type|enum|abstract\s+class)\s+([A-Za-z_$][A-Za-z0-9_$]*)/gm;
        ...
        } catch { /* ignore parse errors in workspace scan */ }

        const isBarrel = /index\.[jt]sx?$/.test(filePath) ||
                         exports.length > 5 && imports.length === 0;

        return {
            filePath, relPath, ext,
            mtime:     stat.mtimeMs,
            exports, imports, regions, lineCount: lines.length,
            isBarrel,
...
export class WorkspaceGraphBuilder {
     ...
     */
    async build(workspaceRoot: string, fileUris: string[]): Promise<WorkspaceGraph> {
        ...
        for (const fp of fileUris) {
            ...
            if (!SUPPORTED_EXTS.has(ext)) { skipped++; continue; }

            // Check if file should be ignored
            const parts = fp.split(/[\\/]/);
            if (parts.some(p => IGNORED_DIRS.has(p))) { skipped++; continue; }
        ...
        this._graph = {
            workspaceRoot, files, symbolExporters,
            builtAt: Date.now(), scannedCount: scanned, skippedCount: skipped,
        };
        return this._graph;
    }

    getGraph(): WorkspaceGraph | null { return this._graph; }
    invalidate(): void { this._graph = null; }
}
...
export class MergeAdvisor {
    ...
    private _score(
    ...
    ): { total: number; reasons: string[]; sharedSymbols: string[]; hasSimilarKind: boolean; alreadyLinked: boolean } {
        let total = 0;
        ...
        const nameMatch = record.exports.some(e =>
        ...
        if (sharedSymbols.length > 0) {
            total += Math.min(0.30, sharedSymbols.length * 0.10);
            reasons.push(`Shares ${sharedSymbols.length} symbol(s): ${sharedSymbols.slice(0, 3).join(', ')}`);
        }

        // 4. Framework alignment
        const regionFramework = regionKind === 'hook' || regionKind === 'react-component' ? 'react' : 'none';
        ...
        return { total: Math.min(1, total), reasons, sharedSymbols, hasSimilarKind, alreadyLinked };
    }
}
