...
const NAMESPACE_REEXPORT_RE = /export\s*\*(?:\s+as\s+\w+)?\s+from\s*['"]([^'"]+)['"]/g;
...
export function extractReExports(filePath: string): ReExportRecord[] {
    ...
    while ((m = NAMED_REEXPORT_RE.exec(src)) !== null) {
        ...
        if (symbols.length > 0) {
            records.push({ fromFile: filePath, specifier, symbols, isNamespace: false });
        }
    }
    ...
    while ((m = NAMESPACE_REEXPORT_RE.exec(src)) !== null) {
        records.push({ fromFile: filePath, specifier: m[1], symbols: ['*'], isNamespace: true });
    }
...
export class ReExportChainResolver {
    ...
    private _addToExporters(graph: WorkspaceGraph, symbol: string, filePath: string): void {
