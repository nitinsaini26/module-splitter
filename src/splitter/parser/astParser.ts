...
function containsHookCalls(node: ts.Node): boolean {
    ...
    const walk = (n: ts.Node): void => {
        ...
        if (ts.isCallExpression(n) && ts.isIdentifier(n.expression)) {
            if (/^use[A-Z]/.test(n.expression.text)) { found = true; return; }
        }
        ts.forEachChild(n, walk);
    };
    walk(node);
    ...
    const walk = (n: ts.Node): void => {
        if (ts.isIdentifier(n)) {
            ...
            if (
                ...
                const text = n.text;
                if (text && !/^[a-z]{1,3}$/.test(text)) { // Skip tiny keywords/vars
                    used.add(text);
                }
            }
        }
...
function extractLeadingComment(node: ts.Node, sf: ts.SourceFile): string | undefined {
...
function buildSymbolTable(sf: ts.SourceFile): SymbolTable {
    ...
    ts.forEachChild(sf, (node) => {
        ...
        if (ts.isImportDeclaration(node)) {
            ...
            if (!clause) {
                imports.set(specifier, { specifier, named: [], isSideEffect: true, line });
                return;
            }

            const rec: ImportRecord = { specifier, named: [], isSideEffect: false, line };
...
function nodeToRegion(
    ...
    const base = {
        startLine, endLine, lines, isExported: exported,
        isDefaultExport: false, leadingComment,
        usedSymbols, localBindings, maxBracketDepth: depth,
    };
    ...
    if (ts.isFunctionDeclaration(node) && node.name) {
        ...
        return {
            ...
            name, hasJSX, hasHooks: hasHk, hasAsyncOps: containsAsync(node),
        };
    }
    ...
    if (ts.isVariableStatement(node)) {
        for (const decl of node.declarationList.declarations) {
            ...
            if (/^[A-Z_][A-Z0-9_]{2,}$/.test(name) || ts.isNumericLiteral(init) || ts.isStringLiteral(init)) {
                return {
                    ...base, id: newId(name),
                    kind: 'constant-block', name,
                    hasJSX: false, hasHooks: false, hasAsyncOps: false,
                };
            }
        }
    }
    ...
    if (ts.isClassDeclaration(node) && node.name) {
        ...
        return {
            ...base, id: newId(name), kind, name,
            hasJSX: containsJSX(node), hasHooks: false,
    ...
    if (ts.isInterfaceDeclaration(node)) {
        return {
            ...base, id: newId(node.name.text),
            kind: 'type-block', name: node.name.text,
            hasJSX: false, hasHooks: false, hasAsyncOps: false,
        };
    }
    ...
    if (ts.isExportAssignment(node) && !node.isExportEquals) {
        ...
        else if (ts.isClassExpression(expr) && expr.name) { name = expr.name.text; }
        ...
        return {
            ...base, id: newId(name),
            kind: classifyName(name, hasJSX, hasHk),
            name, hasJSX, hasHooks: hasHk,
            hasAsyncOps: containsAsync(node),
            isDefaultExport: true, isExported: true,
        };
    }
...
export function parseSourceFile(
    ...
    if (!TS_EXTENSIONS.has(ext)) {
        ...
        return {
            regions: raw.map((r, i) => ({ ...r, id: `rgn_fb_${i}_${r.name}` })) as ASTRegion[],
            symbolTable: {
                locals: new Map(),
                imports: new Map(),
                unresolved: new Set(),
    ...
    for (const region of regions) {
        const entry: SymbolEntry = {
            ...
            namespace: region.kind === 'type-block' || region.kind === 'enum' ? 'type' : 'value',
        };
    ...
    return { regions, symbolTable, parseErrors, engineUsed: 'typescript-ast', aiEnhanced: false };
}
