...
const I = {
    ...
    link:     svg('M10.59 4.59A2 2 0 0012 4h2a2 2 0 010 4h-2a2 2 0 01-1.41-.59L9.17 8.83A2 2 0 019 10v2a2 2 0 11-4 0v-2a2 2 0 01.59-1.41L7 7.17A2 2 0 018 6V4a2 2 0 01.59-1.41z', 16),
...
function makeTabs(plan: SplitPlan): TabDef[] {
    return [
        ...
        { id: 'regions',  label: 'Regions',   icon: I.region,  count: plan.regions.length },
        { id: 'extract',  label: 'Extract',   icon: I.extract, count: plan.extractionCandidates.length },
        { id: 'linkage',  label: 'Linkage',   icon: I.link,    count: plan.linkageMap.length },
        { id: 'smells',   label: 'Smells',    icon: I.smell,   count: plan.codeSmells.length + (plan.frameworkSmells?.length ?? 0) },
        { id: 'tests',    label: 'Tests',     icon: I.test,    count: plan.testFileSuggestions.length },
        { id: 'files',    label: 'Files',     icon: I.files,   count: plan.proposedFiles.length },
...
function metricBar(value: number, max: number, color = 'var(--vscode-focusBorder,#007acc)'): string {
...
function severityBadge(severity: CodeSmell['severity']): string {
    const map: Record<string, 'ok' | 'warn' | 'err' | 'info'> = {
        critical: 'err', high: 'err', medium: 'warn', low: 'info', info: 'info',
    };
...
function renderOverview(plan: SplitPlan): string {
    ...
    const metricCards = [
        ...
        { label: 'Avg Cognitive CC', value: m.avgCognitiveComplexity,        max: 40,   unit: '' },
        { label: 'Maintainability',  value: m.maintainabilityIndex,           max: 100,  unit: '/100' },
        { label: 'Halstead Volume',  value: Math.round(m.halsteadVolume),     max: 5000, unit: '' },
        { label: 'Tech Debt',        value: m.technicalDebtMinutes,           max: 480,  unit: ' min' },
        { label: 'Bundle Weight',    value: Math.round(m.bundleImpactScore),  max: 1000, unit: '' },
        { label: 'Dup Logic Risk',   value: Math.round(m.duplicateLogicRisk * 100), max: 100, unit: '%' },
    ].map(c => `
    ...
    const threshColor    =
        threshPct < 28 ? 'var(--vscode-terminal-ansiGreen,#4caf50)'  :
        threshPct < 42 ? 'var(--vscode-editorInfo-foreground,#3794ff)' :
    ...
    const hitColor    = hitPct > 70 ? 'var(--vscode-terminal-ansiGreen,#4caf50)'
                      : hitPct > 30 ? 'var(--vscode-editorInfo-foreground,#3794ff)'
...
function buildMiniGraphSVG(plan: SplitPlan): string {
    ...
    if (nodes.length === 0) return '<p style="color:var(--vscode-descriptionForeground)">No regions detected.</p>';
    ...
    const edgeLines = plan.dependencyGraph.edges
        ...
        .map(e => {
            ...
            return `<line x1="${f.x}" y1="${f.y}" x2="${t.x}" y2="${t.y}" stroke="${color}" stroke-width="${e.strength * 2}" stroke-opacity="0.6" marker-end="url(#arr)"/>`;
        })
        .join('');

    const nodeCircles = positions.map((p, i) => {
        const r = nodes[i];
        ...
        return `
        <g>
          <circle cx="${p.x}" cy="${p.y}" r="14" fill="${fill}" fill-opacity="0.2" stroke="${fill}" stroke-width="1.5"/>
          <text x="${p.x}" y="${p.y + 4}" text-anchor="middle" font-size="7" fill="${fill}">${esc(short)}</text>
        </g>`;
    }).join('');

    return `
    <svg viewBox="0 0 ${W} ${H}" width="100%" style="max-height:160px;border-radius:6px;background:rgba(0,0,0,0.15)">
      <defs>
        <marker id="arr" viewBox="0 0 8 8" refX="8" refY="4" markerWidth="6" markerHeight="6" orient="auto">
          <path d="M0,0 L0,8 L8,4 z" fill="var(--vscode-focusBorder,#007acc)" opacity="0.7"/>
        </marker>
      </defs>
      ${edgeLines}
      ${nodeCircles}
...
function renderExtract(plan: SplitPlan): string {
    ...
    const cards = plan.extractionCandidates.map((r, i) => {
        const pf = plan.proposedFiles[i];
        if (!pf) return '';
        const importLines = pf.resolvedImports.map(l => `<div style="color:var(--vscode-terminal-ansiCyan,#4ec9b0);font-size:10px">${esc(l)}</div>`).join('');
...
function renderLinkage(plan: SplitPlan): string {
    ...
    const rows = plan.linkageMap.map(link => {
        ...
        <div class="err-row ${circular ? 'sev-error' : critical ? 'sev-warn' : 'sev-info'}">
          ...
          ${circular ? `<div class="err-fix" style="border-left-color:var(--vscode-editorError-foreground,#f14c4c)">
            ⚠ Circular dependency — split or introduce a shared abstraction to break the cycle
          </div>` : ''}
        </div>`;
    }).join('');

    const circularSummary = plan.circularRisks.length > 0
    ...
    const criticalPath = plan.criticalPathFiles.length > 1
...
function renderSmells(plan: SplitPlan): string {
    const allSmellCount = plan.codeSmells.length + (plan.frameworkSmells?.length ?? 0);
    if (allSmellCount === 0) {
    ...
    const tsSection = tsRows ? `<div class="section-label">${I.smell} Code Smells (${plan.codeSmells.length})</div>${tsRows}` : '';

    // Merge suggestions section
    const merges   = plan.mergeSuggestions ?? [];
    const mergeSection = merges.length > 0 ? `
...
function renderTests(plan: SplitPlan): string {
    ...
    const cards = plan.testFileSuggestions.map(t => {
        const testLines = t.suggestedTests.map(s =>
            `<div style="font-size:10px;padding:2px 0;color:var(--vscode-terminal-ansiGreen,#4caf50)">${I.check} ${esc(s)}</div>`
        ).join('');

        const mockLines = t.mockImports.map(m =>
...
function renderFiles(plan: SplitPlan): string {
    ...
    const cards = plan.proposedFiles.map(pf => {
...
function renderDryRun(plan: SplitPlan): string {
    const updatedPreview = plan.updatedSourceContent.split('\n').slice(0, 40).join('\n');
    ...
    const fileList = plan.proposedFiles.map(pf => `
...
.tabs{display:flex;background:var(--vscode-sideBar-background,#1e1e1e);border-bottom:1px solid var(--vscode-panel-border,rgba(127,127,127,.18));flex-shrink:0;padding:0 2px;gap:1px;overflow-x:auto}
.tab{flex:1;display:flex;align-items:center;justify-content:center;gap:3px;padding:7px 2px 6px;font-size:10px;font-weight:500;color:var(--vscode-tab-inactiveForeground,#888);background:transparent;border:none;border-bottom:2px solid transparent;cursor:pointer;white-space:nowrap;min-width:0;letter-spacing:.01em}
...
export function renderSplitPlanHtml(plan: SplitPlan): string {
    ...
    const tabLabels = tabs.map(t =>
        `<label for="tab-${t.id}" class="tab">
           ${t.icon}
           <span>${esc(t.label)}${t.count !== undefined
...
<div class="summary-strip">
  ...
  ${(plan.codeSmells.length + (plan.frameworkSmells?.length ?? 0)) > 0
      ? badge(`${I.smell} ${plan.codeSmells.length + (plan.frameworkSmells?.length ?? 0)} smell${(plan.codeSmells.length + (plan.frameworkSmells?.length ?? 0)) !== 1 ? 's' : ''}`,
              plan.codeSmells.some(s => s.severity === 'critical') ? 'err' : 'warn')
      : ''}
