...
import { buildDependencyGraph, findCriticalPath }      from '../graph/dependencyGraph';
import { computeRegionMetrics, computeFileMetrics }    from '../analysis/metrics';
import { detectRegionSmells, detectFileSmells }        from '../analysis/smellDetector';
...
import { generateFileContent, buildBarrelFile, generateUpdatedSource } from '../generator/fileGenerator';
import { regionCache }                                 from '../cache/regionCache';
import { detectFrameworkSmells, detectFrameworkKind }  from '../frameworks/frameworkPlugins';
...
import type {
    EnrichedRegion, SplitPlan, SplitSummary, WorkspaceContext,
    ProposedFile, FileLinkage, TestFileSuggestion, CodeSmell,
    RegionMetrics, IncrementalCacheStats,
    FrameworkSmellRecord, RegionMergeSuggestions,
} from '../types';
...
const LANG_MAP: Record<string, string> = {
    ...
    mts: 'TypeScript (ESM)', cts: 'TypeScript (CJS)', mjs: 'JavaScript (ESM)', cjs: 'JavaScript (CJS)',
    py: 'Python', java: 'Java', cs: 'C#', go: 'Go', rs: 'Rust',
};
const detectLanguage = (f: string) => LANG_MAP[f.split('.').pop()?.toLowerCase() ?? ''] ?? 'Unknown';
...
function buildLinkageMap(
    ...
    for (const pf of proposedFiles) {
        ...
        for (const dep of region.importedSymbols) {
            const depRegion = enrichedRegions.find(r => r.name === dep && r.id !== region.id);
            ...
            if (existing) { if (!existing.symbols.includes(dep)) { existing.symbols.push(dep); existing.edgeWeight++; } }
            else linkages.push({ from: pf.fileName, to: depFile, symbols: [dep], isCircular: false, isCriticalPath: false, edgeWeight: 1 });
        }
    }
...
function buildSummary(
    regions: EnrichedRegion[], extractionCandidates: EnrichedRegion[],
    proposedFiles: ProposedFile[], totalDebt: number
): SplitSummary {
    const complexity =
        regions.length > 10 && extractionCandidates.length > 5 ? 'highly-complex' :
        regions.length > 6  && extractionCandidates.length > 3 ? 'complex' :
        regions.length > 3  && extractionCandidates.length > 1 ? 'moderate' : 'simple';
    const recommendations: string[] = [];
    if (extractionCandidates.length > 0) recommendations.push(`Extract ${extractionCandidates.length} region(s) to dedicated files.`);
    return {
        totalRegions: regions.length, extractionCount: extractionCandidates.length,
        ...
        recommendation: recommendations.join(' ') || 'File is well-structured — no extraction needed.',
        estimatedRefactorMinutes: totalDebt,
        dryRunPreview: proposedFiles.map(pf => `  ✦ ${pf.fileName} (${pf.estimatedLines} lines, ${pf.resolvedImports.length} imports)`),
    };
}

const DEFAULT_CTX: WorkspaceContext = {
    existingTypeFiles: [], existingHookFiles: [], existingUtilFiles: [],
    existingIndexFiles: [], existingTestFiles: [], sourceDir: '',
    testFramework: 'jest', packageManager: 'npm', isMonorepo: false,
};
...
export class ModuleSplitter {
    analyse(
        ...
        // Stage 1: Parse
        const { regions: rawRegionsBase, symbolTable, engineUsed } = parseSourceFile(sourceCode, fileName);
        ...
        const rawRegions = applyFunctionLevelSplit(
            rawRegionsBase, symbolTable, sourceCode, fileName
        );
        ...
        let semanticInfo = semanticTypeResolver.resolveFile(filePath || fileName, sourceCode);
        ...
        const cyclicRegionIds = new Set<string>(dependencyGraph.sccs.filter(s => s.length > 1).flat());

        // Stages 3+4: Metrics & smells — INCREMENTAL
        const regionSmellMap = new Map<string, ReturnType<typeof detectRegionSmells>>();
        ...
        for (const raw of rawRegions) {
            ...
            if (cached) {
                ...
                const approxCC = 1 + ((src.match(/\bif\b|\bfor\b|\bwhile\b|\b&&\b|\b\|\|\b/g)) ?? []).length;
                const smells   = detectRegionSmells(raw, raw.lines.length, approxCC, raw.maxBracketDepth);
        ...
        const allMetrics          = rawRegions.map(r => metricsMap.get(r.id)!).filter(Boolean);
        ...
        for (const [regionId, metrics] of metricsMap) {
            ...
            if (pfThresh !== undefined) {
                metricsMap.set(regionId, { ...metrics, perFunctionThreshold: pfThresh });
            }
        }
        ...
        const enrichedRegions: EnrichedRegion[] = rawRegions.map(raw => {
            ...
            const couplingScore = dependencyGraph.couplingScores.get(raw.id) ?? 0;
            const cohesionScore = dependencyGraph.cohesionScores.get(raw.id) ?? 0.5;
            const symbolEntry   = symbolTable.locals.get(raw.name);
            const isDeadExport  = !!(raw.isExported && symbolEntry && symbolEntry.referencedByRegionIds.size === 0);

            const exportedSymbols = symbolEntry
                ? [...symbolEntry.referencedByRegionIds].flatMap(rid => {
            ...
            for (const sym of raw.usedSymbols) {
                const ir = [...symbolTable.imports.values()].find(r =>
                    r.defaultAlias === sym || r.namespaceAlias === sym || r.named.some(n => n.alias === sym)
                );
                if (ir && !ir.specifier.startsWith('.') && !externalPackages.includes(ir.specifier)) {
                    externalPackages.push(ir.specifier);
                }
            }

            const inlineTypeNames = [...raw.usedSymbols].filter(sym =>
                symbolTable.locals.get(sym)?.namespace === 'type'
            );

            const codeSmells: CodeSmell[] = smells.map(s => ({ ...s, affectedRegionIds: [raw.id] }));

            // Use per-function threshold if available, otherwise fall back to file threshold
            const regionThreshold = metrics.perFunctionThreshold ?? calibratedThreshold;

            const extractionDecision = evaluateExtraction({
                region: raw, metrics, smellNames: smells.map(s => s.name),
                couplingScore, cohesionScore, isDeadExport, sourceFileExt: ext,
                isInSCC: cyclicRegionIds.has(raw.id),
                calibratedThreshold: regionThreshold,   // ← per-function calibrated threshold
            });

            return {
                ...raw, metrics, smells: codeSmells, exportedSymbols, importedSymbols,
                externalPackages, inlineTypeNames, isDeadExport, extractionDecision,
            };
        });
        ...
        const extractionCandidates = enrichedRegions.filter(r => r.extractionDecision.shouldExtract && r.kind !== 'type-block');
        const retainedRegions      = enrichedRegions.filter(r => !r.extractionDecision.shouldExtract || r.kind === 'type-block');

        // Stages 6+7: Import resolution + file generation
        const proposedFileMap = new Map<string, string>();
        for (const region of extractionCandidates) proposedFileMap.set(region.id, region.extractionDecision.suggestedFileName);

        const proposedFiles: ProposedFile[] = extractionCandidates.map(region => {
            const resolved = resolveImports(region, enrichedRegions, symbolTable, proposedFileMap, typeRouting, effectiveCtx);
            const pf       = generateFileContent(region, region.extractionDecision.suggestedFileName, resolved, fileName, effectiveCtx, typeRouting, enrichedRegions, proposedFileMap);
            pf.hasExistingTest = effectiveCtx.existingTestFiles.some(tf => tf.endsWith(pf.testFilePath));
            return pf;
        });

        // Stage 8: Linkage + critical path
        const linkageMap = buildLinkageMap(proposedFiles, enrichedRegions, proposedFileMap);
        for (const pf of proposedFiles) {
        ...
        const fileAdj           = new Map<string, Set<string>>(proposedFiles.map(pf => [pf.fileName, new Set(pf.linkedTo)]));
        const criticalPathFiles = findCriticalPath(proposedFiles.map(pf => pf.fileName), fileAdj);
        for (const link of linkageMap) {
            link.isCriticalPath = criticalPathFiles.includes(link.from) && criticalPathFiles.includes(link.to);
        }

        const codeSmells           = detectFileSmells(rawRegions, regionSmellMap);
        const barrelExport         = buildBarrelFile(proposedFiles);
        const testFileSuggestions: TestFileSuggestion[] = proposedFiles.map(pf => {
            const region = enrichedRegions.find(r => r.id === pf.sourceRegionId)!;
            return {
                sourceFile: pf.fileName, testFile: pf.testFilePath,
                framework: effectiveCtx.testFramework === 'vitest' ? 'vitest' : 'jest',
                suggestedTests: (pf as unknown as { testFileSuggestions?: string[] }).testFileSuggestions ?? [],
                mockImports: region?.externalPackages.map(pkg => `vi.mock('${pkg}')`) ?? [],
            };
        });
        const fileMetrics      = computeFileMetrics(sourceCode, enrichedRegions);
        const updatedSource    = generateUpdatedSource(lines, extractionCandidates, proposedFiles, fileName);
        const summary          = buildSummary(enrichedRegions, extractionCandidates, proposedFiles, fileMetrics.technicalDebtMinutes);
        ...
        const detectedFramework = detectFrameworkKind(sourceCode, filePath || fileName);
        const rawFrameworkSmells = detectFrameworkSmells(sourceCode, filePath || fileName);
        const frameworkSmells: FrameworkSmellRecord[] = rawFrameworkSmells.map(s => ({
            name:           s.name,
            severity:       s.severity,
        ...
        return {
            sourceFile: fileName, language, totalLines: lines.length, parseEngine: engineUsed,
            symbolTable, dependencyGraph, regions: enrichedRegions, retainedRegions, extractionCandidates,
            proposedFiles, summary, linkageMap, codeSmells, typeRouting, barrelExport,
            testFileSuggestions, circularRisks, criticalPathFiles, metrics: fileMetrics,
            updatedSourceContent: updatedSource, thresholdCalibration, cacheStats,
            frameworkSmells, detectedFramework, mergeSuggestions,
        };
    }
}
