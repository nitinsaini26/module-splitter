...
export function buildTestFile(
    ...
    const setupLine = framework === 'vitest'
    ...
    const renderLine = region.hasJSX
        ...
        : '';
    const hookLine = region.kind === 'hook'
        ? "import { renderHook, act } from '@testing-library/react';"
        : '';

    const tests = ((pf as ProposedFile & { testFileSuggestions?: string[] }).testFileSuggestions ?? []).map((t: string) =>
        `  it('${t}', () => {\n    // TODO: implement test\n    expect(true).toBe(true);\n  });`
    ).concat(
        ((pf as ProposedFile & { testFileSuggestions?: string[] }).testFileSuggestions?.length ?? 0) === 0
...
export function generateFileContent(
    ...
    if (region.kind === 'react-component' || region.kind === 'context-provider') {
        ...
        if (region.hasAsyncOps) testSuggestions.push('handles loading and error states');
    ...
    if (region.smells.length > 0) {
        testSuggestions.push(`no regression: ${region.smells[0].name.toLowerCase()}`);
    }

    // Sort imports: type-only first, then external, then local
    const typeImports  = resolved.statements.filter(s => s.includes('import type'));
    const reactImports = resolved.statements.filter(s => s.includes("from 'react'"));
    const extImports   = resolved.statements.filter(
        s => !s.includes('import type') && !s.includes("from 'react'") && !s.includes("from '.")
    );
    ...
    if (resolved.exportStatements.length > 0 && !body.trimStart().startsWith('export')) {
        parts.push(...resolved.exportStatements, '');
    }

    const generatedContent = parts.filter((p, i) => {
        // Collapse multiple consecutive blank lines
        return !(p === '' && parts[i - 1] === '');
    }).join('\n');

    const barrelEntry = buildBarrelEntry(
        { fileName } as ProposedFile,
        region
    );
