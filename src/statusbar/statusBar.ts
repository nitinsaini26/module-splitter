...
export class AstraStatusBar {
    ...
    update(editor: vscode.TextEditor | undefined): void {
        ...
        try {
            ...
            if (source.trim().length === 0) { this.item.hide(); return; }

            const plan = moduleSplitter.analyse(source, path.basename(editor.document.fileName));
            this._render(plan);
        } catch {
            this.item.hide();
        }
    ...
    updateFromPlan(doc: vscode.TextDocument, plan: SplitPlan): void {
        const cfg = vscode.workspace.getConfiguration('astra');
        if (!cfg.get<boolean>('statusBarEnabled')) { this.item.hide(); return; }
        if (!/\.(ts|tsx|js|jsx)$/.test(doc.fileName)) { this.item.hide(); return; }
        this._render(plan);
    }
