...
import type { SplitPlan, WorkspaceContext } from '../splitter/types';
...
export function registerAnalyseCommand(
    ...
    ctx.subscriptions.push(
        ...
        vscode.commands.registerCommand('astra.analyseFile', async (uri?: vscode.Uri) => {
            ...
            if (uri) {
                ...
                try {
                    ...
                    vscode.window.showErrorMessage(`ASTra: Cannot open file — ${String(e)}`);
                    return;
                }
            } else {
            ...
            await vscode.window.withProgress(
                ...
                async (progress) => {
                    ...
                    const userThresh  = vscode.workspace.getConfiguration('astra')
                                            .get<number>('extractionThreshold') ?? 0.35;

                    progress.report({ increment: 25, message: 'Building workspace graph…' });
                    ...
                    progress.report({ increment: 55, message: 'Computing metrics & smells…' });
                    ...
                    progress.report({ increment: 80, message: 'Publishing diagnostics…' });
...
function _showPanel(plan: SplitPlan): void {
    ...
    _panel.onDidDispose(() => { _panel = undefined; }, null, _ctx.subscriptions);
}
...
async function _findFiles(pattern: string, max: number): Promise<string[]> {
    const ignore = vscode.workspace.getConfiguration('astra')
