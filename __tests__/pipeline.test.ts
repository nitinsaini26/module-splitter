...
import type { WorkspaceContext } from '../src/splitter/types';
...
describe('Stage 2 — buildDependencyGraph', () => {
    it('detects circular dependency via Tarjan SCC', () => {
        const { regions, symbolTable } = parseSourceFile(CIRCULAR_SRC, 'circular.ts');
    ...
    it('produces topological order with no cycles for simple file', () => {
        const { regions, symbolTable } = parseSourceFile(SIMPLE_COMPONENT, 'Counter.tsx');
        const graph = buildDependencyGraph(regions, symbolTable);
...
describe('Stage 3 — Metrics', () => {
    ...
    describe('cognitiveComplexity', () => {
        ...
        it('increments higher for nested ifs', () => {
            ...
            // We test nested > flat
            expect(cognitiveComplexity(nested)).toBeGreaterThanOrEqual(cognitiveComplexity(flat));
        });
    });
    ...
    describe('halsteadMetrics', () => {
        ...
        it('returns higher volume for longer code', () => {
            ...
            const long  = Array.from({ length: 50 }, (_, i) => `const v${i} = ${i} + ${i * 2};`).join('\n');
            expect(halsteadMetrics(long).volume).toBeGreaterThan(halsteadMetrics(short).volume);
        });
    });
...
export function useBad() {
  ...
  useEffect(() => {
    setV(1);
        ...
        const s = smells.find(s => s.name === 'Missing useEffect Dependency Array');
...
describe('Stage 5 — ExtractionOracle', () => {
    const buildInput = (overrides: Partial<Parameters<typeof evaluateExtraction>[0]> = {}) => {
        const { regions } = parseSourceFile(SIMPLE_COMPONENT, 'Counter.tsx');
    ...
    it('always retains type-blocks', () => {
        ...
        const decision = evaluateExtraction({
            region: typeRegion, metrics,
            smellNames: [], couplingScore: 0, cohesionScore: 0.5,
            isDeadExport: false, sourceFileExt: 'ts', isInSCC: false,
        });
    ...
    it('extracts hooks with high affinity score', () => {
        ...
        const decision = evaluateExtraction({
            ...
            couplingScore: 0.5, cohesionScore: 0.7,
            isDeadExport: false, sourceFileExt: 'ts', isInSCC: false,
        });
    ...
    it('produces miDelta > 0 when shouldExtract = true', () => {
        const input = buildInput({ smellNames: ['God Component', 'Large Module (>100 lines)'] });
        input.metrics = { ...input.metrics, lineCount: 150, cyclomaticComplexity: 15, maintainabilityIndex: 40 };
    ...
    it('SCC penalty reduces extraction score', () => {
        const inSCC    = evaluateExtraction(buildInput({ isInSCC: true,  smellNames: [] }));
        const notInSCC = evaluateExtraction(buildInput({ isInSCC: false, smellNames: [] }));
        // If both extract, the SCC one should have lower confidence
        if (inSCC.shouldExtract && notInSCC.shouldExtract) {
            const confMap: Record<string, number> = {
                definitive: 5, high: 4, medium: 3, low: 2, speculative: 1,
            };
            expect(confMap[inSCC.confidence]).toBeLessThanOrEqual(confMap[notInSCC.confidence]);
        }
    });
    ...
    it('suggestedFileName includes correct directory for hooks', () => {
        ...
        const decision = evaluateExtraction({
            region: hook, metrics, smellNames: [],
            couplingScore: 0.3, cohesionScore: 0.6,
            isDeadExport: false, sourceFileExt: 'ts', isInSCC: false,
        });
...
describe('ModuleSplitter — full pipeline', () => {
    ...
    it('type-blocks are NOT in extractionCandidates', () => {
        ...
        const typeExtracted = plan.extractionCandidates.filter((r: { kind: string }) => r.kind === 'type-block');
        expect(typeExtracted.length).toBe(0);
    });

    it('type-blocks appear in typeRouting', () => {
        const plan = splitter.analyse(TYPE_HEAVY, 'types.ts');
        const typeNames = plan.typeRouting.flatMap((t: { typeNames: string[] }) => t.typeNames);
    ...
    it('metrics.overallHealth is a valid grade', () => {
        const plan = splitter.analyse(SIMPLE_COMPONENT, 'Counter.tsx');
        expect(['S', 'A', 'B', 'C', 'D', 'F']).toContain(plan.metrics.overallHealth);
    });
...
describe('renderSplitPlanHtml', () => {
    ...
    it('contains all 8 tab IDs', () => {
        const plan = splitter.analyse(SIMPLE_COMPONENT, 'Counter.tsx');
        const html = renderSplitPlanHtml(plan);
        ['overview', 'regions', 'extract', 'linkage', 'smells', 'tests', 'files', 'dryrun'].forEach(id => {
...
describe('Edge cases', () => {
    ...
    it('handles file with only comments', () => {
        const plan = splitter.analyse('// just a comment\n/* block */\n', 'comments.ts');
        expect(plan.regions.length).toBe(0);
    });
